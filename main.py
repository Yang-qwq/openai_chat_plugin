...
class OpenAIChatPlugin(BasePlugin):
    ...
    async def command_handler(self, event: BaseMessage | GroupMessage | PrivateMessage):
        ...
        if command[0] == '/chat' and len(command) > 1:
            ...
            if command[1] == 'set-present':
                ...
                if target is None:  # 没有指定目标，使用默认配置
                    try:
                        self.data['data'][conversation_dict][event.group_id if event.message_type == 'group' else event.user_id] = \
                        ...
                        display_name = config.plugins_config['openai_chat_plugin']['presents'][present_name].get('display_name', present_name)
    ...
    async def on_load(self):
        ...
        self.register_user_func("命令事件处理器", self.command_handler, prefix='/chat', description="设置插件", examples=["/chat set-present <name> [group:<id>|user:<id>]"])
